...
module mathis {
    ...
    export module creation3D {
        ...
        export class Polyhedron{
            ...
            makeLinks=true
            mamesh:Mamesh
            deformation=new XYZ(1,1,1)
            scale:number
            translation:XYZ
            rotationAxis:XYZ=new XYZ(1,1,0)
            rotationAngle:number=0.543
            private rotationMatrix:MM
            sameSizeAtUnitSphere=true


            constructor(mamesh:Mamesh,type:number){
                this.mamesh=mamesh
                this.type=type
            ...
            go() {
                ...
                var polyhedra:{ vertex: number[][], face: number[][],scale?:number,translation?:XYZ, rotationAxis?:XYZ,rotationAngle?:number}[] = [];
                ...
                polyhedra[Polyhedron.Type.Cube] = {
                    ...
                    scale:2/Math.sqrt(3),
                    translation:new XYZ(-1/Math.sqrt(3),-1/Math.sqrt(3),-1/Math.sqrt(3))
                };
                ...
                let rotationMatrix:MM=null
                if (data.rotationAngle!=null){
                    rotationMatrix=new MM()
                    geo.axisAngleToMatrix(data.rotationAxis,data.rotationAngle,rotationMatrix)
                }
                if (this.rotationAngle!=null){
                    this.rotationMatrix=new MM()
                    geo.axisAngleToMatrix(this.rotationAxis,this.rotationAngle,this.rotationMatrix)
                }

                for (let ve of data.vertex){
                    let vertex=this.mamesh.newVertex(0)
                    vertex.position=new XYZ(ve[0]*this.deformation.x,ve[1]*this.deformation.y,ve[2]*this.deformation.z)
                    if (data.scale!=null) vertex.position.scale(data.scale)
                    if (this.scale!=null) vertex.position.scale(this.scale)

                    if (data.translation!=null) vertex.position.add(data.translation)
                    if (this.translation!=null) vertex.position.add(this.translation)

                    if (rotationMatrix!=null) geo.multiplicationMatrixVector(rotationMatrix,vertex.position,vertex.position)
                    if (this.rotationMatrix!=null) geo.multiplicationMatrixVector(this.rotationMatrix,vertex.position,vertex.position)

                    //vertex.markers.push(Vertex.Markers.corner)
                }
                ...
                for (let face of data.face){
                    ...
                    if (face.length==3) this.mamesh.addATriangle(this.mamesh.vertices[face[0]],this.mamesh.vertices[face[1]],this.mamesh.vertices[face[2]])
                        ...
                        let centerVertex=this.mamesh.newVertex(0)
                        centerVertex.markers.push(Vertex.Markers.pintagoneCenter)
                        centerVertex.position=new XYZ(0,0,0)
        ...
        export module Polyhedron{
            ...
            export enum Type{
                Tetrahedron, Cube, Octahedron,  Dodecahedron, Icosahedron,  Rhombicuboctahedron,  TriangulPrism, PentagonalPrism, HexagonalPrism,  SquarePyramid ,
        ...
        export class SphereFromPolyhedron{
            polyhedronType=Polyhedron.Type.Icosahedron
            makeLinks=true
            mamesh:Mamesh
            dichoLevel=0
            ...
            go():void{
                ...
                let polyhedronCreator=new Polyhedron(this.mamesh,this.polyhedronType)
                polyhedronCreator.makeLinks=false
                polyhedronCreator.go()

                for (let i=0;i<this.dichoLevel;i++){
                    let squareDichotomer=new mameshModification.SquareDichotomer(this.mamesh)
                    squareDichotomer.go()
                    let triangleDichotomer=new mameshModification.TriangleDichotomer(this.mamesh)
        ...
        export class RandomFractal{

            mamesh:Mamesh
            stableLawOptions=new proba.StableLaw.Options()
            referenceDistanceBetweenVertexWithZeroDichoLevel=0.1


            center=new XYZ(0,0,0)

            //TODO add middle in square dicho
            constructor(mamesh:Mamesh){
                this.mamesh=mamesh
            ...
            go(){
                ...
                let simuStable=new proba.StableLaw()
                ...
                let centerToNewPosition=new XYZ(0,0,0)
                while (someThinerDichoLevels){
                    ...
                    for (let key in this.mamesh.cutSegmentsDico){
                        ...
                        if (Math.max(segment.a.dichoLevel,segment.b.dichoLevel)==currentDichoLevel){
                            ...
                            //let distance=geo.distance(segment.a.position,segment.b.position)
                            let modif=X[randomCount++]*Math.pow(this.referenceDistanceBetweenVertexWithZeroDichoLevel/Math.pow(2,currentDichoLevel),1/this.stableLawOptions.alpha)

                            //centerToOldPosition.copyFrom(segment.middle.position).substract(this.center).scale(1+modif)

                            geo.between(segment.a.position,segment.b.position,1/2,centerToNewPosition)
